...
def newTechItem(category_id):
    ...
    if login_session['user_id'] != category.user_id:
        return "<script>function myFunction() {alert('You are not authorized to add tech items to this category. Please create your own category in order to add items.');}</script><body onload='myFunction()''>"

    if request.method == 'POST':
        newTechItem = TechItem(name=request.form['name'], description=request.form['description'],
...
@app.route('/category/<int:category_id>/tech/<int:tech_id>/edit', methods=['GET', 'POST'])
def editTechItem(category_id, tech_id):
    ...
    category = session.query(Category).filter_by(id=category_id).one()
    if login_session['user_id'] != category.user_id:
        return "<script>function myFunction() {alert('You are not authorized to edit tech items to this category. Please create your own category in order to edit items.');}</script><body onload='myFunction()''>"
    ...
    if request.method == 'POST':
        ...
        return render_template('editTechItem.html', category_id=category_id, tech_id=tech_id, item=editedTechItem)

@app.route('/category/<int:category_id>/tech/<int:tech_id>/delete', methods=['GET', 'POST'])
def deleteTechItem(category_id, tech_id):
    ...
    if login_session['user_id'] != category.user_id:
        return "<script>function myFunction() {alert('You are not authorized to delete tech items to this category. Please create your own category in order to delete items.');}</script><body onload='myFunction()''>"
...
def gconnect():
    ...
    # ensure that the token sent to the server matches the token sent to the client
    ...
    try:
        ...
        # specify that this is the one time code flow that the server will be sending off
    ...
    if result['user_id'] != gplus_id:
        response = make_response(json.dumps("Token's user ID doesn't match given ID"), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
